...
void main() {
  runApp(MaterialApp(
    ...
    home: Homepageusermobile(),
  ));
}

  class Homepageusermobile extends StatefulWidget {
  const Homepageusermobile({Key? key}) : super(key: key);
  
  @override
  State<Homepageusermobile> createState() => _MapAppState();
  }

  class _MapAppState extends State<Homepageusermobile> {
  ...
  Text(profileinfo.userEmail, style: TextStyle(color: Colors.white)),
              ...
              ListTile(
                ...
                onTap: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => FavMobile()));
                },
              ),
              ListTile(
...
SizedBox(height: 16.0),
ReviewSlider(
  initialValue: 3,  // Set initial value (adjust as needed)
  options: ['Terrible', 'Bad', 'Okay', 'Good', 'Great'],  // List of labels
  onChange: (int value) {
    _rating = value;  // Update rating based on slider position
  },
),
