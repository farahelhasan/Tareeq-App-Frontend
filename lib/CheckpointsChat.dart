...
class _CommentsState extends State<Comments> {
    ...
    try {
      ...
      if (commentListData['data'] != null) {
        ...
        for (var commentDataItem in commentListData['data']) {
          ...
          String name = await _getUserName(userId);
          int comment_id = commentDataItem['comment_id'];
          String timestamp = commentDataItem['timestamp']; // Assuming the timestamp is returned as a string
          fetchedComments.add({
            'commentId': comment_id,
            'comment': commentDataItem['comment_description'],
            'image': commentDataItem['image_url'],
  ...
  Future<void> _addComment(int checkpointId, String commentDescription, File? image, int userId) async {
    try {
      ...
      String timestamp = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now()); // Add timestamp here

      await ApiService.addCommentController(checkpointId, commentDescription, imageUrl, userId, timestamp); // Pass timestamp
         // to send notification.
      // first: get all users ids who made this checkpoint favorite.
      List<int> userIds = await ApiService.getUserIds(checkpointId);
      for(var userid in userIds){
      // second: get token for each user.
        String token= await ApiService.getToken(userid);    
      // send notification.
        await ApiService.sendFavoriteNotification(token,  profile.name,commentDescription, checkpointinfo.name );
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                String comment = _commentsList[index]['comment'] ?? '';
                String? imageUrl = _commentsList[index]['image'];
                String timestamp = _commentsList[index]['timestamp']; // Get the timestamp
